...
namespace ObjectInfo.Deepdive.SpecificationGenerator.Tests.Dapper.Tests
{
    public class BatchOperationTests : IntegrationTestBase, IClassFixture<DatabaseFixture>
    {
        ...
        public async Task BatchUpdate_ShouldModifyAllMatchingRecords()
        {
            ...
            decimal creditIncrease = 1000m;
            TestSpecification<Customer> spec = new TestSpecification<Customer>(c =>
                c.CustomerType == CustomerType.Regular && c.IsActive);

            string whereClause = spec.GetWhereClause();
            DynamicParameters parameters = new DynamicParameters(spec.GetParameters());
        ...
        public async Task BatchInsert_ShouldInsertAllRecords()
        {
            // Arrange
            int batchSize = 10;
            List<Product> products = Enumerable.Range(0, batchSize)
            ...
            // Verify insertions
            TestSpecification<Product> spec = new TestSpecification<Product>(p => p.Name.StartsWith("Test Product"));

            IEnumerable<Product> insertedProducts = await WithConnection(async conn =>
                await conn.QueryAsync<Product>(
                    $"SELECT * FROM Products WHERE {spec.GetWhereClause()}",
                    spec.GetParameters()));

            insertedProducts.Should().HaveCount(batchSize);
            foreach (Product? product in insertedProducts)
            {
        ...
        public async Task BatchUpsert_ShouldUpdateOrInsertRecords()
        {
            ...
            TestSpecification<Product> spec = new TestSpecification<Product>(p => p.StockLevel < 10);
            IEnumerable<Product> existingProducts = await WithConnection(async conn =>
                await conn.QueryAsync<Product>(
                    $"SELECT * FROM Products WHERE {spec.GetWhereClause()}",
                    spec.GetParameters()));
        ...
        ON CONFLICT(SKU) DO UPDATE SET
            ...
            // Verify upserts
            IEnumerable<Product> updatedProducts = await WithConnection(async conn =>
                await conn.QueryAsync<Product>(
        ...
        public async Task BatchOperations_ShouldBeAtomic()
        {
            ...
            TestSpecification<Order> spec = new TestSpecification<Order>(o => o.Status == OrderStatus.Pending);
            IEnumerable<Order> ordersToUpdate = await WithConnection(async conn =>
            ...
            IEnumerable<Order> unchangedOrders = await WithConnection(async conn =>
                await conn.QueryAsync<Order>(
                    $"SELECT * FROM Orders WHERE {spec.GetWhereClause()}",
                    spec.GetParameters()));
        ...
        private async Task SeedTestDataAsync()
        {
            await WithConnection(async conn =>
            {
                ...
                try
                {
                    ...
                    IEnumerable<int> customerIds = await conn.QueryAsync<int>("SELECT Id FROM Customers", transaction: transaction);
                    ...
                    IEnumerable<int> productIds = await conn.QueryAsync<int>("SELECT Id FROM Products", transaction: transaction);
                    ...
                    IEnumerable<int> orderIds = await conn.QueryAsync<int>("SELECT Id FROM Orders", transaction: transaction);
